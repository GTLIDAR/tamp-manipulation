...
/// Creates and outputs lcmt_object_status messages.
///
...
/// systems::lcm::LcmPublisherSystem::Make<lcmt_object_status>().
///
...
///     comments in `lcmt_object_status.lcm`); when absent, the output message
///     will use zeros.
...
///
/// This system has one abstract-valued output port of type lcmt_object_status.
///
...
///   @output_port{lcmt_object_status}
/// }
/// @see `lcmt_object_status.lcm` for additional documentation.
class ObjectStatusSender : public systems::LeafSystem<double> {
 public:
  DRAKE_NO_COPY_NO_MOVE_NO_ASSIGN(ObjectStatusSender)

  explicit ObjectStatusSender(int num_joints = kObjectNumJoints);
 ...
 private:
  void CalcOutput(const systems::Context<double>&, lcmt_object_status*) const;
