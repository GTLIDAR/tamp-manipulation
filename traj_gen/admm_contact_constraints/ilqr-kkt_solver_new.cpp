...
void ILQR_KKTSolver_new::solveTrajectory()
{
    ...
    for(iter=0;iter<Op.max_iter;iter++)
    {
        ...
        if(newDeriv){
            ...
            dynamicModel->kuka_arm_dyn_cst_ilqr(xList, uListFull, forceListFull, costFunction); // get fx,fu,cx,cu,cxx,cxu,cuu
...
void ILQR_KKTSolver_new::doForwardPass()
{
    ...
    if(!initFwdPassDone){
        ...
        for(unsigned int i=0;i<N;i++) {
            ...
            dynamicModel->kuka_arm_dyn_cst_min_output(updatedxList[i], updateduList[i], updatedforceList[i], isUNan, updatedxList[i+1], costFunction);
            costList[i] = costFunction->getc();
        }
        // getting final cost, state, input=NaN
        isUNan = 1;
        dynamicModel->kuka_arm_dyn_cst_min_output(updatedxList[N], u_NAN_loc, force_unused, isUNan, x_unused, costFunction);
...
ILQR_KKTSolver_new::traj ILQR_KKTSolver_new::getLastSolvedTrajectory()
{
    ...
    lastTraj.time_qp = dynamicModel->time_qp;
    return lastTraj;
}
