...
#include "drake/manipulation_tamp/object_state_estimator.h"
...
class ManipulationStation : public systems::Diagram<T> {
 public:
  ...
  const multibody::ModelInstanceIndex GetConveyorBeltId() {
    DRAKE_THROW_UNLESS(setup_==Setup::kConveyorBelt);
 ...
 private:
  ...
  std::vector<multibody::ModelInstanceIndex> object_model_ids_;
};
