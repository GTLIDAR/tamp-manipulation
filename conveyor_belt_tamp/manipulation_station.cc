...
void ManipulationStation<T>::SetupClutterClearingStation(
  ...
  {
    const std::string sdf_path = FindResourceOrThrow(
        "drake/conveyor_belt_tamp/manipulation_station/models/bin.sdf");
...
void ManipulationStation<T>::SetupManipulationClassStation(
  ...
  // Add the table and 80/20 workcell frame.
  {
    ...
    const std::string sdf_path = FindResourceOrThrow(
        "drake/conveyor_belt_tamp/manipulation_station/models/"
  ...
  // Add the cupboard.
  {
    ...
    const std::string sdf_path = FindResourceOrThrow(
        "drake/conveyor_belt_tamp/manipulation_station/models/cupboard.sdf");
...
void ManipulationStation<T>::SetupPlanarIiwaStation() {
  ...
  {
    const std::string sdf_path = FindResourceOrThrow(
        "drake/conveyor_belt_tamp/kuka_iiwa_arm/models/table/"
        "extra_heavy_duty_table_surface_only_collision.sdf");
...
void ManipulationStation<T>::AddLidarIiwa(
  ...
  auto iiwa_instance = internal::AddAndWeldModelFrom(
      ...
      X_WI, plant_);
  RegisterIiwaControllerModel(
      urdf_path, iiwa_instance, plant_->world_frame(),
      plant_->GetFrameByName("iiwa_link_0", iiwa_instance), X_WI);
}
...
void ManipulationStation<T>::AddLidarWsg() {
  ...
  const multibody::Frame<T>& link_ee =
      plant_->GetFrameByName("iiwa_frame_ee", iiwa_model_.model_instance);
  const RigidTransform<double> X_EC(RollPitchYaw<double>(0, 0, 0),
                                    Vector3d(0, 0, 0));
  internal::AddAndWeldModelFrom(
    connector_urdf_path, "connector", link_ee, "connector_link", X_EC, plant_
  );
...
template class ::drake::conveyor_belt_tamp::manipulation_station::ManipulationStation<
    double>;
